...
export async function GET(request: NextRequest) {
  try {
    ...
    const endpoint = queryString ? `/api/waitlist/entries/?${queryString}` : '/api/waitlist/entries/'
    ...
    const entries = Array.isArray(data) ? data : (data.results || data || [])
    
    return NextResponse.json({ entries })
  } catch (error) {
    console.error("[API] Error fetching waitlist entries:", error)
